...
/** Global variables **/
int oldMain(bool output,FILE *fout,int argc, char *argv[]); //old p1.c
    ...
    */
    ...
    if( argc>1){
        ...
        for(i=1; i<argc; ++i){
            ...
            if(argc==3){ //take in input from screen and writing to file.
                oldmain=false;
                oldMain(oldmain,fout,argc,argv); //passing in false
            }
        ...
        oldmain=true;
        oldMain(oldmain,fout,argc,argv); //passing in true;
    }
    if()
      return 4;
...
int oldMain(bool output,FILE *fout, int argc, char *argv[]){
    ...
    int i; //variable for looping until end of argv
    char startArray[25];
    bool error4=false;
    FILE *openFile;
