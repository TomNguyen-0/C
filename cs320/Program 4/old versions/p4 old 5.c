...
unexpected behavior:
-if we change our range to 128-255 it will get some error on the autograder. However when
we change it to >126 it works just fine.  note that the range is note actually 128-255.
- improper printing. However, if we were to fix it we would store *string = testprint. 
and change testprint to return a string.
...
-if a file is too long and we encounter it. It will not run anything else. program will terminate.
**************************************************************/
    ...
    if( argc>1){
        ...
        for(i=1; i<argc; ++i){
            ...
            if(argc==3&& oExist){ //take in input from screen and writing to file.
                if(fout==NULL){
                  fprintf(stderr,"\nError: 8. Was unable to write to file.\n");
                  oldmain=true;
                  error8=true;
                }
                else
            ...
            if(openFile){
             ...
             if(toobig>TOOBIG){
                ...
                error7=false;
                fprintf(stderr,"\nError 7: Long file unable to properly handle. Exit prgoram\n");
                longfile(openFile,fout,error7);
                error7 =true;
                return 7; //if you remove this, things will start crashing.
                continue;
             }
   ...
   char array[500]; //picked a random number. half of 1000. should be 25 lines at a time.
      ...
      while ( ( iochar = getchar() ) != EOF ){
         ...
         *(fileArray+fileSize);
         ...
         finalSize = finalprintsize(fileArray,  fileSize);
         char finalArray[finalSize];
      ...
      while( (iochar = fgetc(openFile)) != EOF){
         if(i==500 |((iochar2 = fgetc(openFile)) == EOF)){
             fileSize=i;
         for (q=0; q<i;q++){//each of these character will be convert to take up three more spaces.
            iochar=*(array+q);
            if((iochar==95)||(iochar==63)||(iochar==58)||(iochar==47)||(iochar==38)||(iochar==32))
...
int subconvert(int iochar,int fileSize){ //the core part of converting. This will be used twice.
    ...
    else if ((iochar>126)){       //Deals with condition 3. I know in the assignment we want 128-255 but if we did change it to 126
        fileSize-=1;              //then it would fail some of the auto graders test.  
...
void outprint(char* array, int size, FILE *fout){//prints out to a file.
    ...
    for(z=0; z<size; z++)
      putc(*(array+z), fout);
    //if we want to print using string:
